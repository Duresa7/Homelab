...
The cluster as it runs now is described in [Cluster Architecture](../Architecture/Cluster%20Architecture.md).

**Historical scope:** This document records the May 2026 expansion from one node
to three. I checked the four-node state on 2026-07-09 after adding
`red-server` on 2026-07-07. The red node expansion is documented in
[`Red Server Expansion - 2026-07-07.md`](Red%20Server%20Expansion%20-%202026-07-07.md).
The redundant Cluster-Net Corosync link added on 2026-07-10 is documented in
[`Cluster-Net Corosync Link Addition - 2026-07-10.md`](Cluster-Net%20Corosync%20Link%20Addition%20-%202026-07-10.md).
...
All four kept management on `192.168.70.0/24` through `vmbr0.70` and had a dedicated Cluster-Net interface on `192.168.71.0/24` through `vmbr0.71`.
...
- State verified on 2026-07-09: `red-server` was joined as the fourth
  voting node, and the cluster was quorate.
...
`purple-server`, and `blue-server`. By 2026-07-10 the membership also
included `red-server`.
...
State confirmed from all four nodes via the SSH manager on 2026-07-10:
...
`alpha-prod-01`, and `docker-main`) stayed running through the joins.
