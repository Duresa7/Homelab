...
I created the pool on 2026-07-28 with `pvesh create /nodes/purple-server/disks/lvmthin` and restricted the storage entry to Purple. The boot NVMe remains separate. The drive reported SMART `PASSED`, normalized wear 15, and zero reallocated, CRC, or uncorrectable errors after the migration. The [node record's Cluster Storage section](../../../../Hardware/Nodes.md#cluster-storage) explains why node-restricted storage reads `disabled` when `pvesm status` is run on a different node and shows the 2026-08-04 Grey and Purple results side by side.
