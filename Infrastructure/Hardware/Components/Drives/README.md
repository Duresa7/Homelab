...
The tables show only the last four characters of each serial. Current captures replace the full serial and WWN with contextual placeholders before they enter the repository. The tail stays rather than being redacted outright because it is the only thing that tells two identical drives apart: four of my NVMe drives are the same Samsung `MZVLB256HAHQ` part number, and `2896`, `2909`, `5609`, and `5659` are what distinguish their captures. Four characters identify a drive I already own without being the serial anyone could use against a warranty or a support line. I keep a dated file per capture rather than overwriting, named `smartctl-a_<model>_<serial-suffix>_<date>.txt`. `<model>` is the bare part number when the drive has one (`MZVLB256HAHQ`, `CT2000BX500SSD1`), or `Vendor-Model-Capacity` for consumer-branded drives whose part number isn't the name I'd recognize (`Samsung-850EVO-250GB`, `WDC-SN720-512G`). Don't transliterate smartctl's `Device Model` line straight into the filename; on the 850 EVO that produces `Samsung-SSD-850-EVO-250GB`, which splits `850 EVO` and repeats the `SSD` the type folder already carries.

The same rule covers `/dev/disk/by-id` paths, which embed the serial. The [HDD data migration record](../../../../Platforms/Media%20Stack/Documentation/Change%20Records/HDD%20Data%20Migration%20-%202026-07-22.md) writes Red's Seagate ST1000LM035 as `ata-ST1000LM035-1RK172_<REDACTED_DRIVE_SERIAL>`, and the [Media Stack runbook](../../../../Platforms/Media%20Stack/Documentation/Runbook.md) resolves the real path on red-server with a `/dev/disk/by-id/ata-ST1000LM035-1RK172_*` glob.
...
Jedi PC is my Windows 11 workstation, not a Galaxy node, so its drives are listed separately. See [Jedi PC specifications](../../Workstations/Jedi%20PC.md) for the full machine. I ran the 2026-07-24 short self-test from an elevated shell, since Windows blocks the NVMe self-test command without administrator rights.
