...
**Status:** Complete. One change: I restarted `cloudflared` on `edge-01` so it ran the 2026.8.3 binary.  
**Affected systems:** Cloudflare account (four zones, the `edge-01` tunnel, three Access applications), `cloudflared` on `edge-01`

This record holds the 2026-09-06 verification that the [Cloudflare README](../../README.md) carried until 2026-09-25. The Minecraft CNAME and SRV records it lists were deleted on 2026-09-12 with Game 01.

## Public checks

I verified the Cloudflare account twice on 2026-09-06. In the morning the only Executor Cloudflare connection was the `Cloudflare MCP` integration, whose 23 tools cover Workers development and not one of which reads zones, DNS records, the tunnel, or Access, so I verified what public DNS and the connector host can show from `ubuntu-dev` and `edge-01`:
...
- `ts02` and `ts03` are CNAMEs with `_ts3._udp` SRV records on ports 53810 and 49125, matching the [TeamSpeak deployment record](../../../../../Platforms/Teamspeak%20Hosting/Documentation/Deployment.md).
