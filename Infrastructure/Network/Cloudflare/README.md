...
I manage four DNS zones and one Cloudflare Tunnel here. `edge-01` remains the inbound web/application path and is configured from the Cloudflare Zero Trust dashboard rather than a local file. I removed Minecraft’s CNAME and SRV records on 2026-09-12 when I retired Game 01. I keep the live Access application inventory in this component because those policies control administrative entry to Coolify.
...
- `ts02` and `ts03` are CNAMEs with `_ts3._udp` SRV records on ports 53810 and 49125, matching the [TeamSpeak deployment record](../../../Platforms/Teamspeak%20Hosting/Documentation/Teamspeak-deployment.md). Relay targets are withheld here as everywhere.
