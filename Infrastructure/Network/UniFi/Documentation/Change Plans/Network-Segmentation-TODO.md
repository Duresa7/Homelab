...
Zones (REDACTED_PRIVATE_ORG_LABEL-Access, REDACTED_PRIVATE_ORG_LABEL-Security, REDACTED_PRIVATE_ORG_LABEL-Cluster), networks (Access-A/85, Security-A/72, Cluster-Net/71), and the Zone Matrix / firewall policies connecting them are already done. Remaining work is below.
...
- [x] Confirm reachability from both Internal and VPN. Internal DNS and application administration were verified; the VPN-client path into Access-A was validated 2026-07-12 via a NetBird routing peer (see the [change record](../../../../../Platforms/Netbird/Documentation/Change%20Records/NetBird%20First%20Peer%20and%20Routed%20VPN%20Path%20-%202026-07-12.md)).
...
Infrastructure implementation is recorded in [Galaxy Docker-Network LXC Deployment - 2026-07-10](../../../../Compute/Galaxy/Documentation/Change%20Records/Galaxy%20Docker-Network%20LXC%20Deployment%20-%202026-07-10.md). Certificate, proxy-host, HTTPS login, and Compose restart validation are complete; the first-peer enrollment and VPN-client path into Access-A were completed 2026-07-12 and are recorded in the NetBird [change record](../../../../../Platforms/Netbird/Documentation/Change%20Records/NetBird%20First%20Peer%20and%20Routed%20VPN%20Path%20-%202026-07-12.md).

The web and NTP destination addresses remain dynamic because Debian mirrors, container registries, certificate services, and NTP pools can rotate. Least privilege is enforced through the single-host source match, required ports only, ordered catch-all block, and logging rather than a brittle destination-IP list.
...
The operator updated the UniFi console SIEM/syslog destination to `192.168.72.3:1514` on 2026-07-12. A fresh CEF event was processed by SC4S, forwarded through HEC without drops, and indexed in Splunk's `netops` index. No additional Gateway-to-Security policy was required.

### Deferred follow-ups

- [x] Audit and reset the old Wazuh agents: removed disconnected `edge-01` and incorrect `wp-01` registrations; repointed/cleared `app-01` and `edge-01` for operator re-enrollment. `supabase-01` and `alpha-prod-01` have no agent installed; fresh enrollment is tracked in the [Wazuh TODO](../../../../../Platforms/Wazuh/Documentation/TODO.md).
...
The allowed-set decision is resolved: MGMT-A may be reached from Trusted (VLAN 10), Secure (VLAN 50), the existing WireGuard VPN zone, and future NetBird traffic arriving from REDACTED_PRIVATE_ORG_LABEL-Access through its routing peer. Implementing and validating the final MGMT-A block policy is intentionally out of scope for the Security-A migration and remains a separate change.
...
- [x] Verify cluster health after the change (`pvecm status` — all 4 nodes, quorum intact, both links active)
