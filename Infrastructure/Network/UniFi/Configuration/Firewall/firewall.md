...
The gateway runs UniFi's zone-based V2 firewall. I maintain 59 custom policies; UniFi maintains the rest for zone defaults, connection state, return companions, & gateway services.

## Custom Policies

All 59 user-defined policies are enabled, use connection state `ALL`, & run on the `Always` schedule. The live controller held 52 before the 2026-07-26 monitoring relocation, not the 43 this inventory previously claimed. The missing nine were the retained Kasm policies. I added 13 policies for `monitor-01`, including the DNS policy found during execution, deleted six superseded Security-A monitoring policies, and narrowed the retained NPM-to-`security-01` policy to port 443.
...
| Allow NPM to security-01 web UIs | Yes | ALLOW | 10000 | TCP | Both | `<YOUR_ORG_NAME>`-Access | IP 192.168.85.2 | `<YOUR_ORG_NAME>`-Security | IP 192.168.72.2 | 443 | All | Always | On | NPM reaches the Wazuh dashboard only |
...
| Allow Monitor to Personal-A monitoring | Yes | ALLOW | 10000 | TCP | Both | Org-Monitor | IP 192.168.73.2 | Internal | IPs 192.168.40.35, .36, .39, .42 | 9100, 9101 | All | Always | On | Prometheus scrapes node_exporter and cAdvisor on docker-main, ansible-01, docker-blue, and media-01; automatic return policy enabled |
| Allow Monitor to A-Servers monitoring | Yes | ALLOW | 10000 | TCP | Both | Org-Monitor | IP 192.168.73.2 | `<YOUR_ORG_NAME>`-Servers | IPs 192.168.80.10, 192.168.80.118 | 9100, 9101 | All | Always | On | Prometheus scrapes node_exporter and cAdvisor on app-01 and alpha-prod-01; automatic return policy enabled |
| Allow Monitor to A-Access monitoring | Yes | ALLOW | 10000 | TCP | Both | Org-Monitor | IP 192.168.73.2 | `<YOUR_ORG_NAME>`-Access | IP 192.168.85.2 | 9100, 9101, 443 | All | Always | On | Prometheus scrapes docker-network and probes the NPM HTTPS front door; automatic return policy enabled |
| Allow Monitor to A-Security monitoring | Yes | ALLOW | 10000 | TCP | Both | Org-Monitor | IP 192.168.73.2 | `<YOUR_ORG_NAME>`-Security | IPs 192.168.72.2, 192.168.72.3 | 9100, 9101 | All | Always | On | Prometheus scrapes security-01 and splunk-siem; automatic return policy enabled |
| Allow Monitor to DMZ monitoring | Yes | ALLOW | 10000 | TCP | Both | Org-Monitor | IP 192.168.73.2 | Dmz | IP 192.168.90.10 | 9100 | All | Always | On | Prometheus scrapes node_exporter on edge-01; automatic return policy enabled |
| Allow Monitor to Proxmox monitoring | Yes | ALLOW | 10000 | TCP | Both | Org-Monitor | IP 192.168.73.2 | `<YOUR_ORG_NAME>`-Mgmt | IPs 192.168.70.10–.13 | 9100, 8006 | All | Always | On | Prometheus and the PVE exporter reach the Proxmox monitoring and API endpoints; automatic return policy enabled |
| Allow Monitor to Proxmox NUT | Yes | ALLOW | 10001 | TCP | Both | Org-Monitor | IP 192.168.73.2 | `<YOUR_ORG_NAME>`-Mgmt | IPs 192.168.70.10, 192.168.70.13 | 3493 | All | Always | On | The NUT exporter reads `ups01` and `ups02`; automatic return policy enabled |
| Allow Monitor Web Egress | Yes | ALLOW | 10000 | TCP | IPv4 | Org-Monitor | IP 192.168.73.2 | External | Any | 80, 443 | All | Always | On | Permit package, image, certificate, and application web egress from monitor-01 |
| Allow Monitor NTP Egress | Yes | ALLOW | 10001 | UDP | IPv4 | Org-Monitor | IP 192.168.73.2 | External | Any | 123 | All | Always | On | Permit time synchronization from monitor-01 |
| Allow NPM to monitor-01 web UIs | Yes | ALLOW | 10000 | TCP | Both | `<YOUR_ORG_NAME>`-Access | IP 192.168.85.2 | Org-Monitor | IP 192.168.73.2 | 3000, 9090 | All | Always | On | NPM reaches Grafana and Prometheus; automatic return policy enabled |
| Allow Secure to monitor-01 break-glass | Yes | ALLOW | 10000 | TCP | Both | Internal | IP 192.168.50.241 | Org-Monitor | IP 192.168.73.2 | 3000, 9090 | All | Always | On | Jedi PC can reach the two web interfaces directly; automatic return policy enabled |
| Allow Automation to monitor-01 SSH | Yes | ALLOW | 10001 | TCP | Both | Internal | IP 192.168.40.36 | Org-Monitor | IP 192.168.73.2 | 22 | All | Always | On | ansible-01 reaches the restricted SSH account; automatic return policy enabled |
| Allow Monitor DNS to Gateway | Yes | ALLOW | 10000 | All | IPv4 | Org-Monitor | IP 192.168.73.2 | Gateway | Any | 53 | All | Always | On | Permit gateway DNS over TCP and UDP after the initial split-horizon lookup failed |

The `<YOUR_ORG_NAME>`-Access and `<YOUR_ORG_NAME>`-Security egress trios are order-sensitive and use index order 10000, 10001, then 10002. I disabled UniFi automatic respond-policy generation on all six egress entries (`create_allow_respond=false`). The `Org-Monitor` web and NTP egress pair follows the same order without a terminal zone-wide block because the zone defaults already deny unmatched traffic. I enabled automatic response policies for the cross-zone monitoring, NPM, break-glass, and SSH paths.

Two entries in the table above describe the intended state rather than what the controller currently displays, because the plugin can't write either field. `Allow NPM to security-01 web UIs` still carries its pre-relocation description naming Wazuh, Grafana, & Prometheus, though its destination port really is 443 alone; the name should also lose the plural, since one interface is left. `Org-Monitor` should be `<YOUR_ORG_NAME>`-Monitor. Both are UI edits, tracked in the [root backlog](../../../../../TODO.md).
