...
| docker-main | LXC 110 | grey-server | Docker apps | Immich<br>Forgejo<br>Homelab Dashboard<br>Portainer<br>Syncthing |
...
| kasm-01 | VM 122 | purple-server | Isolated disposable desktops (`192.168.78.10`, VLAN 78 control plane) | Kasm Workspaces 1.19.0 CE<br>Docker 29.6.2<br>macvlan session lanes 74, 77, 79 |
...
| Workspace images | No workspace definitions or pulled workspace images; each future image downloads on first launch |
| Session isolation | `lab74`, `lab77`, and `lab79` macvlan networks on addressless VLAN parents; host shims persisted before Docker; the `Lab Sessions` group limits sessions to one hour with upload on and download, clipboard, and persistent profile off |
| Swap | 4 GiB file at `/mnt/Kasm.swap`, required by Kasm's own guidance |
| Network | Static `192.168.78.10/24` on LAB-MGMT/VLAN 78; sessions use `192.168.74.208/28`, `192.168.77.208/28`, or `192.168.79.208/28`; VLAN 74 exits through Proton while VLANs 77 and 79 have no Internet |
...
|---|---|---|---|---|
| docker-main | Upstream binary (Debian 12 bookworm) | `node_exporter.service` | `192.168.40.35:9100` | 9101, 11 containers, `overlay2` |
