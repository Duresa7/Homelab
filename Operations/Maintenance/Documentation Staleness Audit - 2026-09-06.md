...
- **Cloudflare through Executor, at the time.** The Executor Cloudflare connection that existed during the audit exposed Workers development tools and documentation search, so I could not read zones, DNS records, or the tunnel through it. Later in the day I verified the public DNS side by hand, and in the evening I added an account-scoped connection and read the account itself; see the follow-ups below and the [Cloudflare README](../../Infrastructure/Network/Cloudflare/README.md).
...
- **Cloudflare** got a public-DNS and connector-host verification, recorded in the [Cloudflare README](../../Infrastructure/Network/Cloudflare/README.md): nameservers, the tunnel wildcard, the Minecraft and TeamSpeak CNAME and SRV records, and the absence of internal names all check out. In the evening I added the account-scoped [Cloudflare Account MCP](../../Platforms/Executor/Documentation/Change%20Records/Cloudflare%20Account%20MCP%20Integration%20-%202026-09-06.md) connection to Executor and read the four zones and every DNS record, the tunnel configuration, and the Access applications through it. Everything matched, with one finding the public reading could not see: the connector was still running 2026.7.3 from the 2026-08-10 boot, so I restarted it and Cloudflare then reported 2026.8.3 on all four connections. That closed the open decision in the root TODO.
