...
**Scope:** Removal of working files left on `ansible-01`, `monitor-01`, and the five Galaxy nodes after the five-node expansion; reversal of the PXE registry rename; and the SSH client and key-trust parity fixes that brought `green-server` in line with the other four nodes

## Outcome

I cleared 23 leftover files and 4 directories across seven machines, reverted the one live change the cancelled node rename had made, and closed the two SSH parity gaps that kept Green from matching the other four. Galaxy still reports five expected votes, five total votes, and `Quorate`. All seven Proxmox services are active on all five nodes, all 19 guests are running, and Prometheus holds 49 of 49 targets up.

The count breaks down as `ansible-01` 5 files and 2 directories, `red-server` 8 files and 2 directories, `blue-server` 4 files, `purple-server` 2, `green-server` 2, `grey-server` 1, and `monitor-01` 1.
...
| `green-server` | `/var/lib/proxmox-first-boot/proxmox-first-boot`, `/var/log/galaxy-pxe-first-boot.log` | The executed one-use first-boot script and its 59,635-byte log. I had already checked both for credentials, and the join key had removed itself at the end of the run |
| `blue-server` | `/etc/lvm/backup/pve-old-sata`, `/etc/lvm/archive/pve-old-sata_00000-1822880599.vg` | LVM metadata for a volume group that no longer exists. `vgs` returns only `pve` and `pvs` only `/dev/nvme0n1p3` |
...
I also left the four `config-<epoch>.sql.gz` files under `/var/lib/pve-cluster/backup/` on Purple, Blue, Red, and Green. `pvecm add` writes those pre-join pmxcfs snapshots itself, they run 14 to 24 KB, and Grey has none because it created the cluster rather than joining it.
...
That block was dead. `/run/galaxy-pxe-join-key` no longer exists on any node, and `IdentitiesOnly yes` meant Green would offer only that missing key when reaching Grey. It also left Green without the cipher restriction the other four carry. I replaced it with the standard file. All five now hash to `a9b168c6` at mode 640 and 117 bytes:

```text
Ciphers aes128-ctr,aes192-ctr,aes256-ctr,aes128-gcm@openssh.com,aes256-gcm@openssh.com,chacha20-poly1305@openssh.com
```

The second gap was key trust, and it was on Grey rather than Green. Grey is the only node whose `/root/.ssh/authorized_keys` is a regular file; Purple, Blue, Red, and Green all symlink it to `/etc/pve/priv/authorized_keys`. That deviation is deliberate, because it is what scopes the PXE join key to Grey. The cost is that Grey's file is hand-maintained, and it was last touched at 01:15 on 2026-07-31, before Green joined. It held the root keys for Purple, Blue, and Red and not Green's, so `green-server` to `grey-server` root SSH failed with `Permission denied (publickey)` while the other three worked.

I appended Green's existing cluster root key, the same `ssh-rsa ... root@green-server` line already present in `/etc/pve/priv/authorized_keys`, to Grey's file. That is additive and trusts nothing new. Grey now lists ten keys, the join key still appears only there, and `green-server` to `grey-server` returns `grey-server`.
...
Both ran on 2026-05-30 and differ only in the node name and snapshot epoch.
...
All three partitions matched on both runs. The WD copy finished at 01:16:19 and the Toshiba at 02:14:01, each comparing 1,031,168 bytes, 1,073,741,824 bytes, and 254,475,764,224 bytes:
...
Every node reports two `NoMoreNagging` markers in `proxmoxlib.js`. All seven of `pve-cluster`, `corosync`, `pvedaemon`, `pveproxy`, `pvestatd`, `pve-firewall`, and `pvescheduler` are active on all five. `pvesh get /nodes` lists all five online. Blue still runs CT 104, CT 107, and CT 108. `promtool check config` passes on `monitor-01` and the API reports 49 active targets with 49 up.

The `galaxy-pxe` and `tftpd-hpa` services are active on `ansible-01` and `/health` returns `ok` with HTTP 200.

## Still Open

Three items I did not act on. I closed the first two on 2026-08-01; see [Galaxy Cluster PVE 9.2.6 Upgrade and SSH Host Key Seeding](../../Infrastructure/Compute/Galaxy/Documentation/Change%20Records/PVE%209.2.6%20Upgrade%20and%20SSH%20Host%20Key%20Seeding%20-%202026-08-01.md).

**Closed 2026-08-01.** Red still holds `PeaNUT-S03-NUT-Configure-red-server-2026-07-22.txt`, `PeaNUT-S03-NUT-Package-Install-red-server-2026-07-22.txt`, and `PeaNUT-S06-Verification-red-server-2026-07-22.txt` in `/root`, 91 lines across the three. The 65-line install transcript is almost entirely `apt` progress redraws. The [PeaNUT records](../../Platforms/PeaNUT/README.md) already carry the outcomes, including the exit code 3 that the configure transcript shows. I left all three rather than delete evidence from work I wasn't reviewing. Two of the three turned out to be byte-identical to transcripts already captured under undated filenames, so only the configure run needed keeping. I added it to the PeaNUT evidence index and deleted all three from Red.
...
Grey carries `.claude`, `.claude.json`, and `.codex` in `/root`. Those are agent configuration rather than artifacts of this work, and they predate it.
