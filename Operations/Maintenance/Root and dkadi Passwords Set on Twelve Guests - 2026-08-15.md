...
Before the run I confirmed by SHA-256 digest, without reading or printing either value, that the staging file carried the two values I intended and that they were distinct from one another. I had renamed a field in the credential item shortly beforehand, so that check was worth making rather than assuming: it proved the rename had not moved this work onto the wrong value. Which fields those are stays in the unpublished Linux Host Baseline Standard, which is the one file permitted to describe where a host account's credentials come from.
