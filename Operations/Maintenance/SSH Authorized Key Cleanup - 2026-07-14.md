...
I removed two identified ED25519 keys from every readable authorized-key scope where they existed, then rescanned 15 reachable targets and found zero remaining matches. Termix identity `K05` and Proxmox identities `K07` through `K10` matched their recorded fingerprints.
...
Connected-account coverage doesn't prove the absence of keys in unreadable accounts or nonstandard `AuthorizedKeysFile` locations.
