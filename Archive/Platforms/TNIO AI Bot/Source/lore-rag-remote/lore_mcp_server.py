...
def lore_agent_answer(query: str, limit: int = 5, max_seconds: int = AGENT_MAX_SECONDS, session_context: list[dict] | None = None) -> dict:
    ...
    cache_key = f"agent-answer::v32::{corpus}::{clean_query.lower()}::{limit}::{max_seconds}::{ctx_key}"
    ...
    # Cache acceptance gate. Caching a *bad* answer is catastrophic because
    # the same wrong sources get served for the full TTL. We refuse to cache
    # if any signal says the answer is unreliable:
    #   - best_effort: cold-start / template fallback
    #   - confidence == low: the answerer itself flagged low trust
    #   - candidate_count is 0/None: nothing was retrieved at all
    #   - sift failed in a way we cannot vouch for (llm_error, no_json, etc.)
    evidence = response.get("evidence") or {}
    sift_info = evidence.get("sift") or {}
    sift_reason = (sift_info.get("skipped_reason") or "").lower()
    untrusted_sift = sift_reason in {
        "llm_error", "empty_response", "no_json", "bad_shape",
        "json_parse_error", "rejected_all_passthrough",
        "small_pool",  # thin retrieval — likely missed the right doc
    }
    # Independent floor: any answer derived from <8 retrieved candidates
    # is statistically more likely to have missed the right doc. Don't
    # memoize such answers — let the next ask try fresh.
    pre_sift = evidence.get("pre_sift_count")
    thin_retrieval = isinstance(pre_sift, int) and pre_sift < 8
    if (
        response.get("best_effort")
        or response.get("confidence") == "low"
        or evidence.get("candidate_count") in (0, None)
        or untrusted_sift
        or thin_retrieval
    ):
        return response
...
def _legacy_lore_agent_answer(query: str, limit: int = 5, max_seconds: int = AGENT_MAX_SECONDS) -> dict:
    ...
    corpus = cache_namespace()
    cache_key = f"agent-answer::legacy-v31::{corpus}::{clean_query.lower()}::{limit}::{max_seconds}"
    cached = cache_get(AGENT_ANSWER_CACHE, cache_key)
    if cached is not None:
        cached = dict(cached)
        cached["cached"] = True
