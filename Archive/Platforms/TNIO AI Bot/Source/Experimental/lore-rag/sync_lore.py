...
def index_file_chunks(coll, file_id: str, chunks: list[dict]) -> None:
    if not chunks:
        try:
...
def sync() -> int:
    ...
    for file in discovered:
        ...
        if previous.get("modifiedTime") == file.get("modifiedTime") and previous.get("mimeType") == file.get("mimeType"):
            skipped += 1
            continue
        ...
        index_file_chunks(coll, file_id, file_chunks)
        old_files[file_id] = {
            "id": file_id,
            "name": file["name"],
            "path": file.get("path", file["name"]),
