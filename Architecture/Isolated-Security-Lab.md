...
The lab has one control plane and four session lanes. The control plane is reachable from the approved Internal sources and the Management Access VPN. I verified the VPN path from a real remote client on 2026-07-28. Session containers never join the management network.

## Current Topology

`kasm-01` is VM 122 on `purple-server`. Its 200 GiB disk lives on `ssd-lvm2`, the LVM-thin pool backed by Purple's Samsung 850 EVO. The control plane listens at `192.168.78.10` on LAB-MGMT VLAN 78.

VM 122 has four additional VLAN NICs with no host address:

| VLAN | Docker network | Container range | Purpose | Internet |
| ---: | --- | --- | --- | --- |
| 75 KASM-TRUSTED | `lab75` | `192.168.75.208/28` | Trusted coding tools | Ordinary WAN |
...
flowchart LR
    ...
    S["Named Secure workstation"] -->|"TCP 22, 443"| M
    V["Management Access VPN"] -->|"TCP 22, 443"| M
    M -->|"Docker API + local shims"| T75["lab75<br/>KASM-TRUSTED"]
    ...
    T75 -->|"ordinary WAN"| I["Internet"]
    B -->|"Proton, kill-switched"| I
    T75 -. blocked .-> B
    T75 -. blocked .-> D
    T75 -. blocked .-> E
...
5. Per-workspace profile paths limit host persistence to six named trusted or research tiles.
6. Proxmox keeps the whole lab workload on Purple and its replaceable SATA SSD.
...
| --- | --- | --- |
| Trusted, Personal-A, and the named Secure workstation | `192.168.78.10` | TCP 22 and 443 |
| Management Access VPN | `192.168.78.10` | TCP 22 and 443, verified from a remote client |
| LAB-MGMT | External | Resolver and image-pull traffic |
| KASM-TRUSTED | External | Ordinary WAN |
| KASM-BROWSER | External | Through `KASM Lab Proton Egress` |
| KASM-BROWSER | MALWARE-OFFLINE | Tooling may initiate toward a target |

MALWARE-OFFLINE cannot initiate back toward KASM-BROWSER. Neither active lane reaches EVIDENCE-QUARANTINE. KASM-TRUSTED cannot reach any other session lane. Every session lane has an explicit block toward LAB-MGMT, Internal, Servers, Management, Access, and Observability.
...
The `Lab Sessions` group has a one-hour session limit and a two-session ceiling. Browser-mediated uploads and persistent profiles are enabled. Downloads, clipboard in both directions, seamless clipboard, printing, sharing, microphone access, and user storage mappings are disabled.

Persistent profiles are an explicit per-tile choice. Claude Code, Codex CLI, Terminal on lane 75, Nessus, Hunchly, and Telegram each use a separate directory under `/var/lib/kasm-profiles`. Every lane 77 and lane 79 tile keeps the profile path null. A workspace with no Docker network override runs on `kasm_default_network` and gains control-plane egress, so I treat a missing override as a failed workspace definition.
...
- VLAN 75 used the ordinary WAN and could not reach a listening workspace service on VLAN 74, 77, or 79.
...
- A reboot restored all NICs, shims, routes, and Docker networks. The Kasm health endpoint passed, and a fresh lane 77 session launched at `192.168.77.208`.
- Temporary containers, images, firewall rules, and test interfaces were absent after cleanup.

The original containment targets are in the [session isolation record](../Platforms/Kasm%20Workspaces/Documentation/Change%20Records/Kasm%20Session%20Isolation%20-%202026-07-28.md). The workspace, persistence, and four-lane results are in the [workspace build-out record](../Platforms/Kasm%20Workspaces/Documentation/Change%20Records/Kasm%20Workspace%20Build-Out%20-%202026-07-28.md).
...
- I use browser-mediated upload only. I leave download, clipboard, printing, sharing, and storage mappings disabled.
- I assign a persistent path only to a named tile that needs it, and I keep malware and review paths null.
...
- [Kasm Workspace Build-Out change](../Platforms/Kasm%20Workspaces/Documentation/Change%20Records/Kasm%20Workspace%20Build-Out%20-%202026-07-28.md)
- [Kasm Workspaces deployment](../Platforms/Kasm%20Workspaces/Documentation/Deployment.md)
- [UniFi networks and VLANs](../Infrastructure/Network/UniFi/Configuration/network-vlan.md)
- [UniFi firewall zones](../Infrastructure/Network/UniFi/Configuration/zone.md)
- [UniFi firewall policies](../Infrastructure/Network/UniFi/Configuration/firewall.md)
