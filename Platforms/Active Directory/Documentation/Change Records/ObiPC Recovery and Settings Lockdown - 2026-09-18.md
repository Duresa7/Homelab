...
- **My Tier 2 account cannot reproduce any of this.** `DK-t2` is in `ADM-T2-WorkstationAdmins`, which policy makes a local administrator, and every rule collection carries an explicit allow for `S-1-5-32-544`. The Store denies name the restricted group's SID and nothing else. An administrator seeing a working Store is the design behaving correctly, not the policy failing.
