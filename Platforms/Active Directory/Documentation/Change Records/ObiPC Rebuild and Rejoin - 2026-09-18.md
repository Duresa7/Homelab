...
I did not diagnose the cause before reinstalling, and I wrote the first version of this record believing I had corrupted the installation myself. Later the same evening it turned out that `IK-user` had reset the machine from the Windows recovery menu, which Windows 11 allows with no credentials; the reset logs on the new installation confirm a "Remove everything" reset started at 4:19 PM. That is written up as an [incident](../../../../Security/Incidents/Active%20Directory/ObiPC%20Wiped%20from%20the%20Recovery%20Menu%20-%202026-09-18.md), and the controls that close it are in [ObiPC Recovery and Settings Lockdown](ObiPC%20Recovery%20and%20Settings%20Lockdown%20-%202026-09-18.md). Windows reported install time 5:30 PM, which is when the reset finished writing the image.
