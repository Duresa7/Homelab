...
A final configuration comparison initially reported `Deployed config differs` on all six Hawser projects. Comparing field paths showed Compose had supplied `services.hawser-updater.networks` and `services.hawser-updater.command`. I corrected the verification to normalize both the reviewed candidate and deployed file through Compose before comparison. All eleven deployed project definitions matched their reviewed candidates.
