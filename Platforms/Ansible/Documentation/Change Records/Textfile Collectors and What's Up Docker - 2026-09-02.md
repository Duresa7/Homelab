...
- WUD's default tag matching offered `16-rootless` for `forgejo:15`. Containers with variant tags need a `wud.tag.include` label in their own Compose file to narrow the candidates, which is a per-project edit for later.
