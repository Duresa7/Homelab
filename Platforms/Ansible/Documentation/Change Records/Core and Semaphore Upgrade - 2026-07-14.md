...
I upgraded `ansible-01` to Ansible 14.2.0, ansible-core 2.21.2, and Semaphore 2.18.27. I preserved the existing Semaphore project and configured both LXC 100 and the web service to return after a boot. I ran no key-changing task.
...
3. Changed the live Semaphore configuration and SQLite database to mode 0600.
...
| Configuration file modes | Configuration and live database 0600; backup directory 0700 |
...
None required for the upgrade and boot persistence. I rebooted the controller LXC successfully and intentionally did not introduce a full Proxmox-node reboot solely as a test; persisted `onboot: 1` is the Proxmox node-boot control.
