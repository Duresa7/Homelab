...
| [S08-security-01-reboot.log](Logs/S08-security-01-reboot.log) | The approved security-01 run reached the reboot task after an empty apt pass and a confirmed reboot requirement. The SSH tool timed out while the reboot action kept waiting, so this artifact has no recap; S10 proves the guest completed the boot and cleared the requirement. |
| [S09-splunk-siem-reboot.log](Logs/S09-splunk-siem-reboot.log) | The corrected reboot path proved a new splunk-siem boot ID and returned over SSH. Its first system-state gate stopped on the transient failed SC4S health-check unit, preserving the exit-2 result that led to the bounded startup retry. |
| [S10-post-reboot-final-verification.log](Logs/S10-post-reboot-final-verification.log) | Both guests reported no remaining reboot requirement, all 11 guests had zero failed units, Wazuh and cAdvisor passed, and Splunk loaded the newest kernel with healthy SC4S. The final Prometheus jq expression was malformed, so this combined readback ends with exit 2. |
| [S10b-monitoring-post-reboot-verification.log](Logs/S10b-monitoring-post-reboot-verification.log) | The corrected monitoring readback returned 48 of 48 targets up, HTTP 200 from Prometheus and Grafana, and 3 healthy series for each TeamSpeak check, with exit code 0. |
| [S10c-final-reboot-automation-check.log](Logs/S10c-final-reboot-automation-check.log) | The final deployed playbook passed its validator, syntax check, & check mode on both rebooted guests. security-01 and splunk-siem each reported `reboot_required=False` with no failed or unreachable host. |

All timestamps inside the logs are UTC. S05 and S06 retain complete results with summarized command labels; the exact ad hoc command strings were not retained. S08 is intentionally partial because the caller timed out while the old reboot action kept waiting. No command in this evidence set updates a Proxmox node or touches `kasm-01`.
