...
I added TCP/3002 to UniFi policy `Allow NPM to docker-main web UIs`. The policy still permits only NPM at `192.168.85.2` to the approved ports on `192.168.40.35`. A request from `docker-network` to the Open WebUI health endpoint succeeded after the change.
