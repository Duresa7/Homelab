...
On 2026-09-21 I replaced the generated Weebarr password with the password from my selected shared login, keeping username `dkadi`. I used the authenticated `/api/settings/access/local` endpoint and updated credential item `<REDACTED_CREDENTIAL_ITEM_NAME>` through stdin. Its saved password matched the requested source in an in-memory digest comparison. A fresh HTTPS login and the Seerr connection test both passed. No secret value or temporary credential file was written during this change; I retained no separate transcript.
