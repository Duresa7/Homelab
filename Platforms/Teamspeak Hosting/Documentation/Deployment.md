# TeamSpeak Deployment on alpha-prod-01

**Created:** 2026-05-27  
**Last updated:** 2026-09-25

I run two TeamSpeak 3 servers (3.13.8 on 2026-09-25), one shared Playit agent, and TS3 Manager on `alpha-prod-01`. This record maps the VLAN address, container ports, Playit relays, Cloudflare SRV records, ServerQuery allowlists, Compose projects, and boot recovery.
...
Both containers use host networking. Playit forwards only the two UDP voice ports to loopback; ServerQuery and file transfer remain on the LAN.
...
| Property | Value |
|---|---|
| Image | `ghcr.io/playit-cloud/playit-agent:latest` (release 1.0.10 on 2026-09-03; image label 1.0 on 2026-09-25) |
| Container | `playit-agent` |
| Compose project | `playit-agent` |
| Compose file | `~/playit-agent/docker-compose.yml` |
| Network mode | host |

The `dkadi` crontab runs `~/playit-agent/playit-boot-recover.sh` at reboot. The script waits 90 seconds, waits for Docker and `api.playit.gg` DNS, restarts `teamspeak-02` and `teamspeak-03`, then restarts `playit-agent`.
...
TS3 Manager connects to `192.168.80.118:10012` and `192.168.80.118:10013`. It doesn't use either public Playit address.
