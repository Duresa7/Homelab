...
5. List manager agents and confirm the expected endpoint state.

Do not weaken firewall policy to compensate for a failed local service.

## 2026-07-13 Agent Removal Recovery

My 2026-07-13 clean-removal change intentionally deleted `/var/ossec` from `app-01` and `edge-01`. Their former endpoint state no longer exists, so it can't serve as a rollback point.
