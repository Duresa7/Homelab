...
I recorded and verified fresh Wazuh agent installations on `app-01` and `edge-01` after removing the old packages, client data, and manager identities. Verification required no manager, UniFi, or endpoint configuration change.
...
**Observed result:** The workflow created manager IDs `004` and `005` with matching endpoint enrollment state.

**Verification:** Both endpoints had the supported package installed after the workflow completed.

**Evidence:** Step 1 has no command transcript or installation screenshot. Steps 2 and 3 independently verify the endpoint and manager state.

### Step 2: Verify the endpoint services, sessions, and policy path

**Action:** I used read-only SSH checks on both endpoints to inspect the Wazuh agent unit, enabled state, active session, and reachability to manager ports TCP 1514 and 1515. I also inspected the two existing Wazuh policies and their referenced port group through read-only UniFi checks.

**Observed result:** Both services were enabled and active, and each endpoint established its TCP 1514 session to the manager. Both UniFi policies were enabled, their destination remained `192.168.72.2`, and the `Wazuh Ports` group contained only TCP 1514 and 1515.
...
**Verification:** `app-01` at `192.168.80.10` and `edge-01` at `192.168.90.10` both reported agent 4.14.5-1, cluster node `node01`, and state Active.

**Evidence:**

![Both fresh endpoints active](../../Evidence/Endpoint%20Re-enrollment%20-%202026-07-13/Screenshots/S03-Wazuh-Endpoints-Active-2026-07-13.png)
...
No Wazuh work remains for this change. `supabase-01` and `alpha-prod-01` aren't Wazuh endpoints; I removed both from the [Wazuh TODO](../TODO.md) on 2026-07-13.
