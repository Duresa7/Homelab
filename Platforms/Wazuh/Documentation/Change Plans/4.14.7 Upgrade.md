...
**Execution status:** Executed 2026-08-04. Outcome in [Wazuh 4.14.7 Central Upgrade](../Change%20Records/4.14.7%20Central%20Upgrade%20-%202026-08-04.md)  
...
Run Wazuh's documented `apt-get update`, then use the answer settled under Question 2 to prove that each unversioned install command will select exactly `4.14.7-1` and Filebeat-OSS `7.10.2-2`. Do not add `-y`, `--only-upgrade`, a version pin, a conffile force flag, or another option that is absent from the reviewed sources.
...
When `apt-get` prompts about `/etc/wazuh-indexer/jvm.options`, choose the updated package file as Wazuh instructs. Reapply only the local JVM differences captured and settled under Question 5. Do not copy the old file wholesale over the new version.
...
Do not release any agent hold until Step 6.1 passes. Use the roster and commands settled under Question 8. For each of the twelve hosts, finish the following gate before moving to the next host:

1. Record the hostname, starting agent version, source state, and held state.
2. Release only that host's hold with the settled command.
