...
| 3 | 2026-07-13 | The first post-install screenshot showed fresh `edge-01` ID `005` as never connected | Endpoint checks found an active service and an established TCP 1514 session; a refreshed dashboard capture showed both IDs `004` and `005` active | Resolved; transient first-check-in delay |
...
## 3. Fresh edge-01 identity initially showed never connected
