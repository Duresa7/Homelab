...
| `aiproxy.alphasecunited.com` | `192.168.40.179:8317` | HTTP | CLI Proxy API. Request and response buffering and proxy caching are off; proxy read, proxy send, and response-send timeouts are 3,600 seconds. I repointed the upstream from `192.168.40.135` to `ubuntu-dev` on 2026-08-13; five provider authentications are loaded. |
