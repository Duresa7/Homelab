...
I published the existing TS3 Manager interface at `https://ts3-manager.alphasecunited.com` through internal Nginx Proxy Manager. UniFi resolves the name to `192.168.85.2`; NPM forwards HTTP to `192.168.80.118:9000` and presents certificate ID 1.
...
- UniFi had no `ts3-manager.alphasecunited.com` record and no NPM policy to TCP 9000 on `alpha-prod-01`.
...
| Setting | Value |
|---|---|
| Domain | `ts3-manager.alphasecunited.com` |
