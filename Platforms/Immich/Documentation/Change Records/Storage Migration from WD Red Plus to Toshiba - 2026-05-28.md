...
I moved Immich from a 4 TB WD Red Plus to a 2 TB Toshiba drive through ZFS and Proxmox. After the move, the timeline loaded, albums and named faces remained present, full-resolution photos opened, videos played, and the web UI returned HTTP 200. I then destroyed the old pool and removed the WD drive.
...
The database holds albums, faces, tags, favorites, shared links, and search metadata, while the original photos and videos live in separate directories. I had to move both from the same stopped state.
...
The replacement Toshiba DT01ACA200 had prior use. SMART reported `PASSED`, zero reallocated sectors, zero pending sectors, and zero CRC errors. Its high power-on hours and load-cycle count kept it from being a long-term single point of failure, so I retained the WD copy until the post-move checks passed.
...
Before moving data, I captured a fresh database backup. The file copy alone wouldn't provide a separate restore point for albums, named faces, and other database-held metadata.
...
I installed the Toshiba in grey-server, wiped it, and initialized a GPT label. In the Proxmox UI, I created single-disk ZFS pool `hddpool-1` and left `Add Storage` enabled so Proxmox registered it as a storage backend.

I used the drive's persistent device path instead of a transient `/dev/sdX` name.

The pool reported `ONLINE`, mapped to the Toshiba device, showed the expected usable capacity, and appeared in the Proxmox storage configuration.
...
In the Immich interface, the timeline loaded, albums retained their covers, named people remained present, full-resolution photos opened, and videos played.
...
I rebuilt the video transcode cache, confirmed playback, and removed the WD drive from grey-server.
