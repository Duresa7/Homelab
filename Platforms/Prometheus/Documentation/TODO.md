...
**Move Prometheus and Grafana off `grey-server`.** `security-01` sits on the node that also holds `app-01`, `docker-main`, and `splunk-siem`, so Grey failing takes monitoring down along with most of what it monitors. Ten steps, the six firewall rules that carry the collector's address, and the rollback point are in [Move Monitoring off grey-server](Change%20Plans/Move%20Monitoring%20off%20grey-server.md). Sequenced after alerting on purpose.
