...
**The admin sign-in is local, not directory.** Portal administrator accounts live in the portal's SQLite `admins` table. The one enabled administrator is `dkadi`, and no account by that name exists in the directory. Version 0.3.0 has no LDAP bind, no Kerberos, no directory synchronization and no AD-group catalog assignment, so a portal username that looks like a person is a label checked against a password hash. Directory sign-in would be a feature, and storing a directory password in the portal would not be one.
