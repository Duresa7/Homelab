...
def summarize_fields(fields: dict) -> str:
    if not fields:
        return ""
    preferred = []
    for key in ("profile_type", "full_name", "role_or_description", "role", "rank", "title", "titles", "notes", "factions", "allegiances", "homeworld", "birthplace", "species", "name", "starship", "ship", "ships", "vehicle", "droid", "droids", "beast", "beasts", "artifact", "artifacts", "equipment", "style", "class", "type", "model", "description", "requirements", "restricted_ownership", "control", "climate", "location", "phase", "outcome", "hyperlane_route_s", "landscape", "roll", "rolls", "bonus", "hull", "hyperdrive", "additional_notes"):
        if fields.get(key):
...
def answer_from_records(query: str, hits: list[dict]) -> dict | None:
    ...
    if wants_list and any(h.get("record_type") == "roster" for h in hits):
    ...
    if wants_list and re.search(r"\bpraetorian officers?\b", q):
        ...
        for record in load_records():
            ...
            if not re.search(r"\b(legatus|praefectus|tribune|prefect)\b", rank_text):
                continue
            if "praetorian" not in text and record.get("source", {}).get("title") != "Know Your Empire":
                continue
    ...
    if top.get("record_type") in {"planet", "rule", "asset", "roster", "entity"} and top.get("relevance_score", 0) >= 3.0:
        ...
        if person_lookup_query(query) and top.get("record_type") == "rule" and not re.search(r"\b(rule|rules|ability|abilities|form|forms|combat|saber|rank|requirement|requirements|roll|dice)\b", q):
            return None
        if person_lookup_query(query) and profile_record_score(top) >= 2:
            return None
        ...
        if normalized_detail in {f"role: {normalized_name}", f"role: {normalized_subject}"}:
            return None
        cite = add_source(top)
        return {"status": "answered", "answer": f"{top.get('name')}: {detail}. {cite}", "sources": sources, "confidence": "high"}
    return None
