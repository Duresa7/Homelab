...
from lore_config import CHUNKS_PATH, COLLECTION_NAME, DEFAULT_SEARCH_LIMIT, EMBED_MODEL, INDEX_DIR, MANIFEST_PATH, OLLAMA_BASE_URL, RECORD_MANIFEST_PATH, STATE_DIR
...
def lore_agent_answer(query: str, limit: int = 5, max_seconds: int = AGENT_MAX_SECONDS, session_context: list[dict] | None = None) -> dict:
    ...
    cache_key = f"agent-answer::v2::{corpus}::{clean_query.lower()}::{limit}::{max_seconds}::{ctx_key}"
