...
Nothing else. Jellyfin user `Ilyas` sees 0 of the 27 anime episodes, which I checked because it looked like a library access gap and is not one: both users still hold `EnableAllFolders`, and `Ilyas` carries `MaxParentalRating` 14, which also filters that account to 137 of 286 television episodes and 4 of 6 movies. That is a deliberate setting and I left it alone.
