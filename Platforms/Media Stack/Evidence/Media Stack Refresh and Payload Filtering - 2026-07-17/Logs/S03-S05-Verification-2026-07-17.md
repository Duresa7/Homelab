...
The live Compose environment and application configuration contain secrets and were not printed. The VPN exit check retained only pass/fail status; the provider address was not captured.
...
- VPN exit lookup from the Gluetun namespace: `passed`; address deliberately not retained.
...
The initial capture occurred while the filter list contained 73 patterns. The final researched baseline was then reconciled to 100 patterns. A subsequent API read returned:
...
That follow-up request exited `0` with empty standard error. The exact 100-pattern list is retained in the [research record](../../../Documentation/Download%20Payload%20Filtering%20Research%20-%202026-07-17.md).

## Persistence Verification

qBittorrent was restarted once more after the final 100-pattern reconciliation:
...
The post-refresh containers were checked from their own network namespaces without reading or printing stored API keys or passwords:
