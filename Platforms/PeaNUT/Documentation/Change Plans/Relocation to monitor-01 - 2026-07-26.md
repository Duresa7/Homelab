...
**Status:** Completed on 2026-07-26. The observed results are in [PeaNUT Relocation to monitor-01 - 2026-07-26](../Change%20Records/Relocation%20to%20monitor-01%20-%202026-07-26.md).

I executed every step including the Nginx Proxy Manager cutover in Step 6, which this plan had reserved as a separate manual step. The Trusted (VLAN 10) zone question in Step 4 resolved to Internal, so `zone.md` was the file that needed correcting.
...
The work splits in two. I build and verify the new instance and open the firewall paths, then pause at Step 6 to repoint Nginx Proxy Manager by hand in its web UI. Teardown and documentation follow after that.

## Baseline verified on 2026-07-26

I checked all of this live before writing the plan. I re-confirm anything a step depends on at run time rather than trusting these lines blindly, but nothing here should have moved.
...
- Retrieve them through the password manager's CLI, piping each value rather than printing it.
...
### Step 6: Pause for the Nginx Proxy Manager cutover

**This step is a manual edit in the NPM web UI.**

Steps 1 to 5 end once Step 5 passes and the Step 4 additions are in. Before touching NPM I note what was verified and the direct URL. Nothing in Steps 1 to 5 touches NPM, and Step 7 waits for this step.
...
| `Platforms/PeaNUT/Documentation/Change Records/Relocation to monitor-01 - 2026-07-26.md` | New record. Observed results and post-change verification, not a restatement of intent |
| `Platforms/PeaNUT/Documentation/Change Plans/Relocation to monitor-01 - 2026-07-26.md` | This file. Status becomes Completed with a link to the record |
...
- Pause after Step 5 and the Step 4 additions. Nginx Proxy Manager changes only in Step 6, by hand.
- Do not remove anything from `docker-main` until I've either finished the NPM cutover or decided to proceed without it.
