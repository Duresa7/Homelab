...
I installed Debian `nut-server` 2.8.1-5 on both hosts. The package also installed `nut-client` and the USB driver dependencies. [Red installation transcript](../../Evidence/UPS%20Dashboard%20Deployment%20-%202026-07-22/Logs/S03-NUT-Package-Install-red-server.txt) and [Grey installation transcript](../../Evidence/UPS%20Dashboard%20Deployment%20-%202026-07-22/Logs/S03-NUT-Package-Install-grey-server.txt) retain the package operations and exit status.
...
I added two TCP/3493 accepts to `pve_mgmt`: `docker-main` at `192.168.40.35` can reach only Grey at `192.168.70.10` and Red at `192.168.70.13`. `pve-firewall compile` returned exit code 0 and rendered both rules into `GROUP-pve_mgmt-IN`. The [firewall transcript](../../Evidence/UPS%20Dashboard%20Deployment%20-%202026-07-22/Logs/S04-Proxmox-Firewall.txt) retains the compiled result.
