...
The first Compose health check used `CMD-SHELL` to run a Node request against `/api/ping`. The web process was running, but every probe failed before Node started. The [initial deployment transcript](../../Evidence/UPS%20Dashboard%20Deployment%20-%202026-07-22/Logs/S05-Dashboard-Deploy.txt) records the exact Compose command, image pull, container start, 90-second timeout, and exit code 124.
...
Docker reported `state=running health=healthy`. `/api/ping` returned `pong`, and the authenticated [dashboard capture](../../Evidence/UPS%20Dashboard%20Deployment%20-%202026-07-22/Screenshots/S06-PeaNUT-Dashboard-After.png) showed both UPS devices online.
