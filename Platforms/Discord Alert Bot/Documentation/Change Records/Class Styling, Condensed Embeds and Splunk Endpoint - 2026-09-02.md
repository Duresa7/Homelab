...
The first real Updates batch then failed for three and a half hours, because a silence link built from fifteen labels passed Discord's field limit, and the first real Splunk post failed because Splunk's results link used a bare hostname. Both are in [Discord Rejected Every Updates Batch](../Troubleshooting/Discord%20Rejected%20Every%20Updates%20Batch%20-%202026-09-03.md). The bot now measures every field, keeps the embed under 6000 characters, and validates URLs. A build went live at 2:07 AM on 2026-09-03 with those fixes. A review later that morning found that the size-trimming function could loop forever when an embed's fields alone exceeded the limit, so it was rewritten without the loop and the VirusTotal permalink was added to the fields the Splunk embed shows; the bot's footer and image name also stopped saying homelab. The final build went live at 4:00 AM with `alert_bot.py` at SHA-256 `c299838035dfd8534fc091a252ca89e28335a3a9b3d4df0ed0f9ef9c7771ab67`, matching the repository.
