...
I use this reference for the UniFi to SC4S to Splunk pipeline in [Build Log.md](Build%20Log.md#step-6-unifi-log-ingestion-cef-via-sc4s). The format comes from Ubiquiti's *UniFi System Logs and SIEM Integration* documentation.
...
| Internet | Outage and Failover, Performance | WAN Failover, High Latency, Packet Loss |
...
These are Ubiquiti's published sample events, not events from my controller. The names (`Craig`, `Office UDM Pro`, `Lobby AP`, `Employee WiFi`) are Ubiquiti's. In the first sample I show the source address as the documentation address `192.0.2.10`.

**Admin Accessed UniFi Network**

```
CEF:0|Ubiquiti|UniFi Network|9.3.33|544|Admin Accessed UniFi Network|1|UNIFIcategory=System UNIFIsubCategory=Admin UNIFIhost=Office UDM Pro UNIFIaccessMethod=web UNIFIadmin=Craig src=192.0.2.10 msg=Craig accessed UniFi Network using the web. Source IP: 192.0.2.10
```
