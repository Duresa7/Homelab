...
- [x] 2026-08-30: Added Wazuh. A Universal Forwarder on `security-01` ships `/var/ossec/logs/alerts/alerts.json` to `192.168.72.3:9997` into the `wazuh` index at sourcetype `wazuh:alerts`, everything forwarded, 30 days retained, no backfill. See [Alert Forwarding to Splunk - 2026-08-29](../../../Wazuh/Documentation/Change%20Records/Alert%20Forwarding%20to%20Splunk%20-%202026-08-29.md).
...
- [x] 2026-09-03: The stale manager identity exclusion in `Wazuh - Machines gone quiet, listed` expires on its own. It matches only that identity when its last event predates 2026-09-02, while a host enrolled under the same name later would still be reported. Proven both ways: the search returns 0 rows, and the same search with the cutoff moved to 2026-09-01 returns the stale identity. The first draft put the test after `table` had dropped the `last` field and excluded nothing; the test sits in the first `where` now.
