...
- [x] 2026-09-03: The `wazuh-01` exclusion in `Wazuh - Machines gone quiet, listed` now expires on its own. It matches only a `wazuh-01` row whose last event predates 2026-09-02, so the renamed manager's stale name is ignored while it ages out and a host genuinely enrolled under that name later would still be reported. Proven both ways: the search returns 0 rows, and the same search with the cutoff moved to 2026-09-01 returns `wazuh-01`. The first draft put the test after `table` had dropped the `last` field and excluded nothing; the test sits in the first `where` now.
