...
I published the control plane at `kasm.alphasecunited.com` through NPM rather than installing a certificate on Kasm itself. The build narrative is in [Deployment](../Deployment.md); what belongs here is why the decision flipped and what I checked afterward.
...
NPM generated `data/nginx/proxy_host/23.conf` with `$forward_scheme https` to `192.168.78.10:443`, the wildcard certificate, block-exploits, force-SSL, and no access list.

HSTS needs a correction I only caught by reading the live headers rather than the config. NPM adds none: no proxy host config on the server contains an `add_header Strict-Transport-Security` line, and the `map $scheme $hsts_header` block in `23.conf` is unused boilerplate NPM writes regardless. Kasm's own nginx sends `Strict-Transport-Security: max-age=63072000` on its own, which I confirmed by requesting `https://192.168.78.10/` directly and seeing the header come back from `Server: nginx`. So the browser does receive HSTS for the proxied name, pinned for two years, and it arrives from Kasm rather than from the proxy policy. That is harmless here because the route forces SSL anyway, but it means the name cannot be served over plain HTTP for two years in any browser that has seen it once. The websocket headers matter more than the rest, since Kasm streams sessions over websockets and a proxy without them serves a login page and then a black screen: `proxy_set_header Upgrade`, `proxy_set_header Connection`, and `proxy_http_version 1.1` are present at both server level and inside `location /`. The login page loads over the proxied name with no certificate interstitial, which also unblocked browser automation against the UI for the first time.

A live session through the proxy settles the part the config could only imply. I launched `Terminal - VPN` from `https://kasm.alphasecunited.com` and the desktop rendered, so the KasmVNC websocket carries through NPM rather than being stripped. Typing reached the container, which proves the socket works in both directions and not just outbound. Inside that session `eth0` held `192.168.74.208/24` and the egress check returned the Proton exit `185.98.168.20`, so proxying the control plane changes nothing about lane assignment or lane egress. I destroyed the session afterward and confirmed zero sessions, eight containers, and all six profile directories back at 4 KB.
...
One cosmetic side effect: the session Control Panel gains an "Install PWA" entry on the proxied origin that the bare-IP origin never offered, because a valid certificate on a real name makes the deployment installable. It shifts the other rows down, which matters only if I am clicking by position rather than by label.
