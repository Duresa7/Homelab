...
| Trigger | Planned maintenance: moving blue, purple, red, and grey onto a UPS |
| Status | Resolved |
| Severity | SEV-2 - Service Outage |

## Incident Summary

A planned maintenance shutdown of `blue-server` at 16:30 EDT took two HA-managed containers offline and the HA manager couldn't bring either one back. I was moving blue, purple, red, and grey onto a UPS, so blue went down on purpose. CT 107 `docker-network` (NetBird control plane and Nginx Proxy Manager) and CT 108 `docker-blue` (the RustDesk relay containers `hbbs` and `hbbr`) were both down for about 33 minutes, from 16:30 to 17:03 EDT.
...
| Area | Impact |
|------|--------|
| CT 107 `docker-network` | NetBird control plane and Nginx Proxy Manager offline from 16:30 to 17:03 EDT. Internal reverse-proxy hosts and the NetBird management/signal path were unreachable during the window. |
| CT 108 `docker-blue` | RustDesk relay (`hbbs` ID/rendezvous and `hbbr` relay) offline for the same window. |
...
| Data loss | None. Both root disks stayed intact on blue-server at their recorded 32 GiB and 15 GiB sizes; the containers shut down cleanly before the node went down. |
| Security exposure | None observed. The two config files moved only between node directories inside the cluster filesystem; no external access and no config edits beyond the recovery. |
...
- CT 107 and CT 108 stayed stopped after blue rejoined at 16:36; their configs sat on purple-server.
- purple's `vzstart` for both failed with `no such logical volume pve/vm-107-disk-0` (and `vm-108-disk-0`).
- HA parked both services in `error`. `ha-manager migrate ct:107 blue-server` returned exit 255 with `service 'ct:107' in error state, must be disabled and fixed first`.
- Nginx Proxy Manager hosts, the NetBird management and signal path, and the RustDesk relay were unreachable from 16:30 to 17:03.
...
| 16:30:12 | blue's `pve-guests` ran `stopall`; all VMs and CTs on blue stopped cleanly |
| 16:30:14 | blue's HA LRM logged `got shutdown request with shutdown policy 'conditional'` then `shutdown LRM, stop all services` |
| 16:30:24 | blue's LRM stopped `ct:107` and `ct:108` (`vzshutdown` tasks) |
| 16:30-16:36 | blue offline for the UPS move; the CRM relocated both configs to purple-server |
| 16:31:34-16:32:14 | purple's HA LRM `vzstart` for 107 and 108 failed repeatedly: `no such logical volume pve/vm-107-disk-0` |
| 16:31:55 | HA-driven `vzmigrate` of 107 and 108 aborted with `found stale volume copy on node 'red-server'` |
...
| 17:03:49-17:03:54 | Both containers started on blue after the manual fix (`vzstart:107` and `vzstart:108` returned `TASK OK`) |
...
`datacenter.cfg` carries no `ha:` line, so the HA `shutdown_policy` is the default `conditional`. Under `conditional`, a node reboot freezes its HA services until the node returns, but a node shutdown relocates them to another node. blue's `last` log shows `shutdown system down Mon Jul 20 16:30 - 16:36`, an actual power-down for the UPS move rather than a reboot, and blue's LRM logged the policy by name at 16:30:14. The shutdown told the CRM to move 107 and 108 off blue.
...
`storage.cfg` defines `local-lvm` as `lvmthin thinpool data vgname pve` with no `nodes` restriction, so every node has a `pve/data` pool, but each pool is separate node-local storage rather than one shared volume. The cluster has no shared block storage reachable by blue, purple, or red; the only non-local pools, `ssd-lvm1` and `hddpool-1`, are restricted to grey-server. So purple received the configs but had nothing to start:
...
- Data corruption or loss: blue's disks were intact at 32 GiB and 15 GiB and mounted cleanly on start.
...
| Action | Status | Notes |
|--------|--------|-------|
| Confirmed disk location and integrity | Complete | `vm-107-disk-0` (32 GiB) and `vm-108-disk-0` (15 GiB) present on blue's `pve/data`, inactive |
| Removed both resources from HA | Complete | `ha-manager remove ct:107` and `ct:108`, both rc 0; cleared the `error` state |
...
| Started both on blue | Complete | `pct start 107` and `108`; task logs returned `TASK OK` at 17:03:49-17:03:54; disks reactivated (`Vwi-aotz--`) |
...
| Verified CT 108 workloads | Complete | 192.168.40.39/24 up, gateway reachable, `hbbs` and `hbbr` running |
| Re-added both to HA | Complete | `ha-manager add ct:107 --state started` and `ct:108`, both adopted `started` on blue |
...
With the strict pin in place, a future blue shutdown or reboot leaves 107 and 108 stopped until blue returns, then they auto-start on blue. That's the correct behavior while their storage is node-local: the HA manager can no longer strand the config on a diskless node.

## Follow-Up Controls

**State on 2026-09-24:** neither container is an HA resource any more, and the `pin-blue-local-storage` rule is gone. `ha-manager config` and `ha-manager rules config` both returned nothing. The removal happened between 2026-09-06 and 2026-09-23 and is recorded in [CT 107 and CT 108 HA Removal](../../../Infrastructure/Compute/Galaxy/Documentation/Change%20Records/CT%20107%20and%20CT%20108%20HA%20Removal%20-%202026-09-24.md). With no HA resource, the stranding path in this report no longer exists. The controls below applied while both containers were HA-managed.

1. Keep the `pin-blue-local-storage` rule while 107 and 108 stay on `local-lvm`. Removing it re-enables the stranding path.
2. With the pin in place, a future blue shutdown or reboot leaves 107 and 108 stopped until blue rejoins, then they auto-start on blue. The HA manager no longer relocates them to another node, which is the intended behavior while their storage is node-local.
...
Resolved on 2026-07-20: both containers ran on blue from 17:03 EDT and the pin was verified at 17:15:26 EDT. The exposure ended when both left HA, recorded in Follow-Up Controls above.
...
- [Galaxy Docker-Network LXC Deployment](../../../Infrastructure/Compute/Galaxy/Documentation/Change%20Records/Docker-Network%20LXC%20Deployment%20-%202026-07-10.md)
