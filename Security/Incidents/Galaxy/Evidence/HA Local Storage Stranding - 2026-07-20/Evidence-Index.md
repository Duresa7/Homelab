...
**Mechanism:** Proxmox web UI screenshots plus `journalctl`, `ha-manager`, `pct`, `lvs`, and `/var/log/pve/tasks` transcripts pulled over SSH from the Galaxy nodes
...
**S02:** Cluster log detail, 16:37-16:38 and 17:03, showing `hamigrate:107`/`hastart:108` ending with `command 'ha-manager ...' failed: exit code 255` and the `vzstart:107` recovery task starting on blue-server.
...
**S03:** Server view after recovery, 107 and 108 back under blue-server, with `CT 108 - Start` and `CT 107 - Start` tasks reporting OK at 17:03. This is the recovered state.
...
| [purple-vzmigrate-aborted-2026-07-20.txt](Logs/purple-vzmigrate-aborted-2026-07-20.txt) | The HA-driven `vzmigrate` of 107 and 108 to red-server aborting, including the `stale volume copy on red-server` note |
