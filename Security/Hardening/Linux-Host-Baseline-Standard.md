...
4. **Authorized keys.** Install exactly these three public keys into `/home/REDACTED_USER_001/.ssh/authorized_keys` (file `0600`, dir `0700`, owned by `REDACTED_USER_001`):
   ...
   *Why: these are the three approved administrative endpoints (two operator machines and the Ansible control node); public keys are safe to record here.*
...
- **Passwordless sudo is a deliberate fleet decision.** The SSH Manager MCP and the `ansible-control` node run privileged commands without a stored sudo password. Requiring a sudo password would force that secret into automation, which is why NOPASSWD is standard here. Hosts with a materially higher risk profile may deviate — record the deviation and its reasoning in that host's own record.
- **Reference implementation.** The `docker-network` LXC (CTID 107) was built to this baseline; its [S03 hardening evidence](../../Platforms/Netbird/Evidence/Docker-Network%20Access%20Stack%20Deployment%20-%202026-07-10/Logs/S03-SSH-HA-Hardening-2026-07-10.md) is a concrete worked example of every command.
- **Automation is the natural next step.** These actions are currently performed per host. A cloud-init snippet or Ansible playbook that enforces this baseline would belong under `Engineering/Automation/` and reference this standard as its specification.
