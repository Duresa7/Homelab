...
| HYG-02 | Critical | `Allow VPN to `<YOUR_ORG_NAME>`-Servers` precedes the Temp VPN block. It would shadow that block if Temp VPN were enabled. Temp VPN is currently disabled, so this is a latent conflict rather than a current active path. |
